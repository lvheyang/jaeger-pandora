...
func TestWriteSpan(tester *testing.T) {
	...
	writer, _ := NewLogzioSpanWriter(PandoraConfig{AccountToken: testAccountToken, CustomListenerURL: server.URL}, logger)
	assert.NoError(tester, writer.WriteSpan(span))

	time.Sleep(time.Second * 6)
	requests := strings.Split(string(recordedRequests), "\n")
	var logzioSpan objects.JaegerSpan
	...
	var logzioService objects.JaegerService
